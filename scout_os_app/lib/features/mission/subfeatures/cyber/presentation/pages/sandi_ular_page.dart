...
class _SandiUlarPageState extends State<SandiUlarPage>
  ...
  /// Build the vertical snake grid from input text (Column-by-Column filling)
  void _buildGrid() {
    ...
    // Fill column by column with vertical snake pattern
    int textIndex = 0;
    for (int col = 0; col < cols; col++) {
      if (col % 2 == 0) {
        // Even column: Fill DOWNWARDS (Row 0 to Max)
        ...
        // Odd column: Fill UPWARDS (Row Max to 0)
  ...
  void _buildGridFromEncrypted() {
    ...
    // Now extract text column by column with vertical snake pattern
    _calculateDecryptedText();
  }

  /// Calculate decrypted text by reading column by column with vertical snake pattern
  void _calculateDecryptedText() {
    ...
    // Read column by column with vertical snake pattern
    for (int col = 0; col < cols; col++) {
      if (col % 2 == 0) {
        // Even column: Read DOWNWARDS (Row 0 to Max)
        for (int row = 0; row < rows; row++) {
          result.add(_grid[row][col]);
        }
      } else {
        // Odd column: Read UPWARDS (Row Max to 0)
        for (int row = rows - 1; row >= 0; row--) {
          result.add(_grid[row][col]);
        }
      }
        ...
        Row(
          ...
          children: [
            ...
            _buildLegendItem(
              ...
              label: 'Odd Columns (↑)',
              color: Colors.amber.shade300, // Light Yellow
            ),
          ],
        ),
      ],
  ...
  Widget _buildGridCell({
    ...
    required int row,
    required int col,
    required bool isEvenCol,
    required Animation<double> animation,
  }) {
    final cellColor = isEvenCol
        ? Colors.lightBlue.withOpacity(0.2) // Light Blue
        : Colors.amber.shade300.withOpacity(0.2); // Light Yellow

    final borderColor = isEvenCol
