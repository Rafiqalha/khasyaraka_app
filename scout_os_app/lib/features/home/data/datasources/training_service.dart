...
class TrainingService {
  ...
  Future<Map<String, dynamic>> fetchLevelById(String levelId) async {
    try {
      final url = Uri.parse(
        '${Environment.apiBaseUrl}/training/levels/$levelId',
      );
      ...
      final response = await http
          ...
          .timeout(
            ...
            onTimeout: () => throw Exception(
              'Connection timeout: Server tidak merespons dalam ${Environment.connectTimeout}ms',
            ),
          );
      ...
      throw Exception(
        'NetworkException: Tidak dapat terhubung ke server - ${e.message}',
      );
    } on FormatException catch (e) {
      throw Exception(
        'JSON Parse Error: Response dari server tidak valid - ${e.message}',
      );
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> fetchLevelsByUnit(String unitId) async {
    try {
      final url = Uri.parse(
        '${Environment.apiBaseUrl}/training/units/$unitId/levels',
      );
  ...
  Future<List<TrainingQuestion>> fetchQuestionsByUnit(String unitId) async {
    try {
      final url = Uri.parse(
        '${Environment.apiBaseUrl}/training/units/$unitId/questions',
      );
  ...
  Future<List<TrainingQuestion>> fetchQuestions(String levelId) async {
    try {
      final url = Uri.parse(
        '${Environment.apiBaseUrl}/training/levels/$levelId/questions',
      );
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
          '   📊 Backend returned: ${allQuestions.length} total questions',
        );
        debugPrint(
          '   ✅ After filtering: ${filteredQuestions.length} questions match levelId',
        );
        if (allQuestions.isNotEmpty) {
          final uniqueLevelIds = allQuestions.map((q) => q.levelId).toSet();
          debugPrint(
            '   📋 Found levelIds in response: ${uniqueLevelIds.join(", ")}',
          );
        ...
        if (filteredQuestions.isEmpty && allQuestions.isNotEmpty) {
          ...
          final uniqueLevelIds = allQuestions.map((q) => q.levelId).toSet();
          throw Exception(
            'Backend returned ${allQuestions.length} questions but none match level "$levelId". Found levelIds: ${uniqueLevelIds.join(", ")}',
          );
        }
  ...
  Future<Map<String, dynamic>> submitProgress({
    ...
    correctQuestionIds, // ✅ NEW: List of question IDs answered correctly
    int timeSpentSeconds = 0,
  }) async {
    try {
      final url = Uri.parse(
        '${Environment.apiBaseUrl}/training/progress/submit',
      );
      ...
      if (token != null && token.isNotEmpty) {
        ...
        debugPrint(
          '✅ [SUBMIT_PROGRESS] Authorization header added: Bearer ${token.substring(0, token.length > 20 ? 20 : token.length)}...',
        );
      } else {
        debugPrint(
          '⚠️ [SUBMIT_PROGRESS] WARNING: No JWT token found! Request will fail with 401 Unauthorized.',
        );
        throw Exception(
          'Not authenticated: JWT token not found. Please login again.',
        );
      }

      debugPrint('📤 [SUBMIT_PROGRESS] Sending request to: $url');
      debugPrint(
        '📤 [SUBMIT_PROGRESS] Payload: level_id=$levelId, score=$score, correct_answers=$correctAnswers, total_questions=$totalQuestions',
      );
      debugPrint(
        '📤 [SUBMIT_PROGRESS] Correct question IDs: $correctQuestionIds',
      );
          ...
          .post(
            ...
            body: json.encode({
              ...
              'correct_question_ids':
                  correctQuestionIds, // ✅ NEW: Send list of correct question IDs
      ...
      debugPrint(
        '📥 [SUBMIT_PROGRESS] Response status: ${response.statusCode}',
      );
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
          '✅ [SUBMIT_PROGRESS] Response parsed: xp_earned=$xpEarned, total_xp=$totalXp',
        );

        return responseData;
      } else if (response.statusCode == 401) {
        debugPrint(
          '❌ [SUBMIT_PROGRESS] 401 Unauthorized - Token may be invalid or expired',
        );
        throw Exception('Unauthorized: Please login again (HTTP 401)');
      } else {
        debugPrint(
          '❌ [SUBMIT_PROGRESS] HTTP ${response.statusCode}: ${response.reasonPhrase}',
        );
  ...
  Future<Map<String, dynamic>> fetchLearningPath(String sectionId) async {
    try {
      final url = Uri.parse(
        '${Environment.apiBaseUrl}/training/sections/$sectionId/path',
      );
      ...
      if (token != null && token.isNotEmpty) {
        headers['Authorization'] = 'Bearer $token';
        debugPrint(
          '✅ [FETCH_PATH] Authorization header added for user progress',
        );
      } else {
        debugPrint(
          '⚠️ [FETCH_PATH] No token - returning generic (non-user) progress',
        );
      }
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          '404: Section "$sectionId" tidak ditemukan atau tidak aktif',
        );
      } else if (response.statusCode == 500) {
        throw Exception('Server Error (500): Database atau Redis bermasalah');
      } else {
  ...
  // Method incrementHearts removed: Security enforcement (AdMob SSV only)

  /// Decrement user hearts on the backend (atomic via Redis)
  ///
  /// Endpoint: POST /api/v1/users/{userId}/hearts/decrement
  ///
  /// Returns: Map with { user_id, hearts, max_hearts, synced }
  Future<Map<String, dynamic>> decrementHearts({
    required String userId,
    int amount = 1,
  }) async {
    try {
      final url = Uri.parse(
        '${Environment.apiBaseUrl}/users/$userId/hearts/decrement',
      );
      ...
      final response = await http
          .post(url, headers: headers, body: json.encode({'amount': amount}))
          .timeout(
            Duration(milliseconds: Environment.connectTimeout),
            onTimeout: () => throw Exception('Connection timeout'),
          );
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('💔 [HEARTS] Decremented: hearts=${data['hearts']}');
        return data;
      } else {
        debugPrint('❌ [HEARTS] HTTP ${response.statusCode}: ${response.body}');
        throw Exception('HTTP ${response.statusCode}');
      }
    } catch (e) {
      debugPrint('❌ [HEARTS] Error decrementing hearts: $e');
      rethrow;
    }
  }

  /// Get user hearts with regeneration check
  ///
  /// Endpoint: GET /api/v1/users/{userId}/hearts
  ///
  /// Returns: Map with { user_id, hearts, max_hearts, next_regeneration_time }
  Future<Map<String, dynamic>> getHearts({
    required String userId,
    bool forceRefresh = false,
  }) async {
    try {
      final url = Uri.parse('${Environment.apiBaseUrl}/users/$userId/hearts');
      ...
      if (response.statusCode == 200) {
        ...
        final data =
            responseData['data'] as Map<String, dynamic>? ?? responseData;
        debugPrint(
          '💚 [HEARTS] Retrieved: hearts=${data['hearts']}, next_regeneration=${data['next_regeneration_time']}',
        );
        return data;
      } else {
        debugPrint('❌ [HEARTS] HTTP ${response.statusCode}: ${response.body}');
      ...
      debugPrint('❌ [HEARTS] Error fetching hearts: $e');
      rethrow;
    }
  }

  /// [DEBUG ONLY] Increment hearts via debug endpoint
  /// This simulates what the AdMob SSV callback would do.
  /// The endpoint returns 404 in production.
  Future<void> debugIncrementHearts({required String userId}) async {
    try {
      final url = Uri.parse(
        '${Environment.apiBaseUrl}/users/$userId/hearts/debug-increment',
      );
      ...
      if (response.statusCode == 200) {
        debugPrint('🧪 [DEBUG] Hearts incremented via debug endpoint');
      } else {
        debugPrint(
          '❌ [DEBUG] Debug increment failed: HTTP ${response.statusCode}',
        );
        throw Exception('HTTP ${response.statusCode}');
      }
    } catch (e) {
      debugPrint('❌ [DEBUG] Error calling debug-increment: $e');
      rethrow;
    }
  }
  ...
  Future<List<Map<String, dynamic>>> fetchSections() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return sectionsJson.map((s) => s as Map<String, dynamic>).toList();
      } else if (response.statusCode == 500) {
        throw Exception('Server Error (500): Database atau Redis bermasalah');
      } else {
        throw Exception(
