...
class _SandiKimiaPageState extends State<SandiKimiaPage> {
  ...
  // Logic State
  bool _isEncryptMode = true; // true = Text to Chemical, false = Chemical to Text
  ...
  void _backspace() {
    ...
    if (selection.isValid && selection.start > 0) {
      if (!selection.isCollapsed) {
          ...
          newText = currentText.replaceRange(selection.start - 1, selection.start, '');
      ...
      appBar: AppBar(
        title: Row(
          ...
          children: [
            ...
            Text("SANDI KIMIA", style: GoogleFonts.fredoka(color: Colors.white, fontWeight: FontWeight.bold)),
          ],
        ),
        centerTitle: true,
        backgroundColor: _bgDark,
                 ...
                 _buildUnifiedCard(
                    ...
                    child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintText: _isEncryptMode ? "Ketik pesan..." : "Gunakan tombol di bawah...",
                          hintStyle: GoogleFonts.fredoka(color: Colors.grey.shade400),
                          contentPadding: EdgeInsets.zero,
                        ),
                        maxLines: 3,
                        minLines: 1,
                        readOnly: !_isEncryptMode, // Read only in decode mode (use custom keyboard)
                    ),
                 ),
                 ...
                 _buildUnifiedCard(
                    ...
                    child: Container(
                       ...
                       child: _chemicalResult.isEmpty 
                         ? Center(child: Text("Hasil akan muncul di sini...", style: GoogleFonts.fredoka(color: Colors.grey.shade400)))
                         : SelectableText(
                            _chemicalResult,
                            style: GoogleFonts.fredoka(
                               fontSize: 22, 
                               color: _isEncryptMode ? _sciencePurple : _textBlack, 
  ...
  Widget _buildChemicalKeyboard() {
     return Container(
       ...
       child: SafeArea( // Ensure safety on bottom devices
          top: false,
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
               ...
               Row(
                 children: [
                   ...
                   _buildExpandedKeyButton(icon: Icons.backspace, color: Colors.redAccent, onTap: _backspace),
                 ],
               ),
               ...
               Row(
                 children: [
                   ...
                   _buildExpandedKey("SPASI", _sciencePurple, flex: 3, value: " "), // Space
                 ],
               ),
            ],
          ),
  ...
  Widget _buildExpandedKey(String label, Color color, {String? value, int flex = 1}) {
    return Expanded(
      flex: flex,
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 2), // Tighter 2px margin
  ...
  Widget _buildExpandedKeyButton({IconData? icon, required Color color, required VoidCallback onTap, int flex = 1}) {
    return Expanded(
      flex: flex,
      child: Padding(
  ...
  Widget _buildKeyButton({String? label, IconData? icon, required Color color, required VoidCallback onTap}) {
     return GestureDetector(
        onTap: onTap,
        child: Container(
           ...
           decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(12),
              boxShadow: [
                 BoxShadow(color: color.withValues(alpha: 0.5), offset: const Offset(0, 3), blurRadius: 0)
              ]
           ),
           alignment: Alignment.center,
           child: icon != null 
    ...
    while (i < morse.length) {
      ...
      if (char == '.') {
        ...
        if (count > 1) {
          ...
          if (subscript != null) result.add('$atom$subscript');
          else result.add(atom * count);
        } else {
          result.add(atom);
        }
      } else if (char == '-') {
