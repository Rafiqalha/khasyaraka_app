...
class RumputToolPage extends StatefulWidget {
  ...
  const RumputToolPage({super.key, required this.sandi});
  ...
  void _onBackspacePressed() {
    setState(() {
      ...
      if (_currentLetterBuffer.isNotEmpty) {
        ...
        _currentLetterBuffer = _currentLetterBuffer.substring(
          0,
          _currentLetterBuffer.length - 1,
        );
        if (_rawMorseSequence.isNotEmpty) {
          _rawMorseSequence = _rawMorseSequence.substring(
            0,
            _rawMorseSequence.length - 1,
          );
        }
      }
      // 2. Jika buffer kosong, tapi ada teks yang sudah didecode
        ...
        _rawMorseSequence = _rawMorseSequence
            .trimRight(); // Hapus spasi separator token/word
                    ...
                    child: _isEncryptMode
                        ...
                        : TextField(
                            controller: TextEditingController(
                              text:
                                  _decodedText +
                                  (_currentLetterBuffer.isNotEmpty
                                      ? '...'
                                      : ''),
                            ), // Show hint if typing
                            readOnly: true,
        ...
        Container(
          decoration: BoxDecoration(
            ...
            boxShadow: const [
              BoxShadow(
                color:
                    _grassGreen, // Match Morse vibrancy with Grass Green Accent shadow
                offset: Offset(0, 6),
                blurRadius: 0,
              ),
            ],
        ...
        child: Column(
          children: [
            ...
            Row(
              children: [
                Expanded(
                  child: _buildGrassKey(
                    "PENDEK",
                    Icons.arrow_drop_up,
                    _grassGreen,
                    _onShortGrassPressed,
                  ),
                ),
                const SizedBox(width: 8),
                Expanded(
