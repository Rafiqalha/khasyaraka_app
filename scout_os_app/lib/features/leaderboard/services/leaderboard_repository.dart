    ...
    try {
      ...
      try {
        ...
        if (token != null && token.isNotEmpty) {
          debugPrint('✅ [LEADERBOARD_REPO] Token found: length=${token.length}, prefix=${token.substring(0, token.length > 20 ? 20 : token.length)}...');
        } else {
          debugPrint('⚠️ [LEADERBOARD_REPO] No token found in SharedPreferences');
        }
      } catch (e) {
        debugPrint('⚠️ [LEADERBOARD_REPO] Error checking token: $e');
      }
      
      debugPrint('📊 [LEADERBOARD_REPO] Fetching leaderboard (limit: $limit)...');
      ...
      debugPrint('✅ [LEADERBOARD_REPO] Response status: ${response.statusCode}');
      debugPrint('🔍 [LEADERBOARD_REPO] Response headers: ${response.headers.map}');
      
      // Backend returns: { "success": true, "data": {...}, "message": "..." }
      final responseData = response.data as Map<String, dynamic>;
      
      debugPrint('📊 [LEADERBOARD_REPO] Raw response type: ${responseData.runtimeType}');
      debugPrint('📊 [LEADERBOARD_REPO] Raw response keys: ${responseData.keys.toList()}');
      ...
      if (responseData['success'] == true && responseData['data'] != null) {
        ...
        debugPrint('✅ [LEADERBOARD_REPO] Response structure OK: success=true, data is Map');
        debugPrint('🔍 [LEADERBOARD_REPO] Data keys: ${data.keys.toList()}');
        debugPrint('📊 [LEADERBOARD_REPO] Parsing data: top_users=${data['top_users']?.length ?? 0}, my_rank=${data['my_rank'] != null ? 'present' : 'null'}');
        ...
        if (data['top_users'] != null) {
          if (data['top_users'] is List) {
            ...
            if ((data['top_users'] as List).isNotEmpty) {
              ...
              debugPrint('📊 [LEADERBOARD_REPO] First user raw data: $firstUser');
              debugPrint('📊 [LEADERBOARD_REPO] First user type: ${firstUser.runtimeType}');
              if (firstUser is Map) {
                debugPrint('📊 [LEADERBOARD_REPO] First user keys: ${firstUser.keys.toList()}');
              }
            } else {
              debugPrint('⚠️ [LEADERBOARD_REPO] top_users is empty list');
            }
          } else {
            debugPrint('❌ [LEADERBOARD_REPO] ERROR: top_users is not a List! Type: ${data['top_users'].runtimeType}');
          }
        } else {
        ...
        if (data['my_rank'] != null) {
          debugPrint('✅ [LEADERBOARD_REPO] my_rank is present, type: ${data['my_rank'].runtimeType}');
          if (data['my_rank'] is Map) {
            debugPrint('📊 [LEADERBOARD_REPO] my_rank keys: ${(data['my_rank'] as Map).keys.toList()}');
        ...
        debugPrint('✅ [LEADERBOARD_REPO] Parsed successfully: topUsers=${leaderboardData.topUsers.length}, myRank=${leaderboardData.myRank != null ? 'present' : 'null'}');
        
        if (leaderboardData.topUsers.isNotEmpty) {
          debugPrint('   Top user: ${leaderboardData.topUsers[0].name} - ${leaderboardData.topUsers[0].xp} XP (rank #${leaderboardData.topUsers[0].rank})');
        } else {
          debugPrint('⚠️ [LEADERBOARD_REPO] WARNING: Parsed topUsers is empty!');
        }
        
        // ✅ CRITICAL: Debug myRank parsing
        if (leaderboardData.myRank != null) {
          debugPrint('✅ [LEADERBOARD_REPO] My rank parsed successfully: rank=${leaderboardData.myRank!.rank}, xp=${leaderboardData.myRank!.xp}');
          
          // ✅ Validate: rank should be >= 1 if xp > 0
          if (leaderboardData.myRank!.xp > 0 && leaderboardData.myRank!.rank == 0) {
            debugPrint('❌ [LEADERBOARD_REPO] ERROR: myRank has XP=${leaderboardData.myRank!.xp} but rank=0! Backend should return rank >= 1.');
          }
          
          // ✅ Validate: rank should be >= 1
          if (leaderboardData.myRank!.rank < 1) {
            debugPrint('❌ [LEADERBOARD_REPO] ERROR: myRank.rank=${leaderboardData.myRank!.rank} is < 1! This should not happen.');
          }
        } else {
          debugPrint('⚠️ [LEADERBOARD_REPO] My rank is NULL - user might not be in leaderboard or not authenticated');
          
          // ✅ Debug: Check raw data
          if (data['my_rank'] != null) {
            debugPrint('⚠️ [LEADERBOARD_REPO] WARNING: Raw data has my_rank but parsing failed! Raw: ${data['my_rank']}');
          } else {
            debugPrint('⚠️ [LEADERBOARD_REPO] Raw data does not have my_rank field');
          }
        }
        ...
        throw Exception('Invalid response format from leaderboard API: success=${responseData['success']}, data=${responseData['data']}');
      }
